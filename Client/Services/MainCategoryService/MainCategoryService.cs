namespace BlazorApp.Client.Services.MainCategoryService
{
    public class MainCategoryService : IMainCategoryService
    {
        ...
        {
            var newCategory = new MainCategory { IsNew = true, Editing = true };
        ...
        public async Task UpdateCategories(MainCategory category)
        {
            var response = await _http.PutAsJsonAsync("api/MainCategory/admin", category);
            AdminMainCategories = (await response.Content.ReadFromJsonAsync<ServiceResponse<List<MainCategory>>>()).Data;
