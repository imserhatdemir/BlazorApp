namespace BlazorApp.Server.Services.ProductService
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ServiceResponse<Product>> GetProductAsync(int productId)
        {
            ...
            var product = await _context.Products.Include(p => p.Variants.Where(v=>v.Visible && !v.Deleted)).ThenInclude(v => v.ProductType).FirstOrDefaultAsync(p => p.ID == productId && !p.Deleted && p.Visible);
            if(product == null)
            {
